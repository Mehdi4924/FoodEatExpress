...
class Mycart extends Component {
    ...
    componentDidMount = async () => {
        ...
        //'https://pizzakebab.ranglerztech.website/api/get-tax'
                ...
                if (dataa.status == 200 && dataa.successData && dataa.successData.length > 0) {
                    dataa.successData.map(async item => {
                        ...
                        if(thesubtotalingprice>=parseInt(item.min_spend)){
                            let actualdiscount = (item.percentage_off * thesubtotalingprice) / 100
                            //this.setState({ texs: actualtax })
                            globaldiscount = actualdiscount;
                            //ToastAndroid.show(JSON.stringify(globaldiscount),ToastAndroid.SHORT)
                        }
                    })
                    //Alert.alert(
                    //'Success',
        ...
        let thetotalprice = 0;
        //ToastAndroid.show(JSON.stringify(thediscount),ToastAndroid.SHORT)
        thetotalprice = parseInt(thedeliverycharges) + (thetax) + parseInt(thesubtotalingprice) -(thediscount)
        this.setState({ subTotal: thesubtotalingprice })
        this.setState({ totalprice: thetotalprice })
                                        ...
                                        >
                                            ...
                                            <View style={{ marginHorizontal: responsiveHeight(2) }}>
                                                <View style={{ width: responsiveWidth(60) }}>
                                                    <Text numberOfLines={1} ellipsizeMode='tail'>{value.name}</Text>
                                                </View>
                                                {
