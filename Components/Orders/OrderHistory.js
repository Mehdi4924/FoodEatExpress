...
export default class Categories extends Component {
  ...
  componentDidMount = async () => {
    ...
    fetch(`${myurl}/api/order-history`, requestOptions)
  ...
  cancel = async (item) => {
    ...
    fetch(`${myurl}/api/cancel-order`, requestOptions)
      .then((response) => response.text())
      .then(async (result) => {
        let dataa = JSON.parse(result);
          ...
          <View style={Styles.DateView}>
            ...
            <View style={{width: '80%'}}>
              <Text style={[Styles.MediumText, {color: Pink}]}>
                {/* {'£'} */}
                {item.branch && item.branch.country === 'pakistan'
                  ? 'PKR '
                  : '£ '}
                {item.total}
              </Text>
      ...
      <View style={Styles.MainCard}>
        <View style={Styles.InnerCardView}>
          <View style={Styles.DateView}>
            ...
            <View style={{width: '80%'}}>
              <Text style={[Styles.MediumText, {color: Pink}]}>
                {console.log('check branch status', item.branch)}
                {item.branch && item.branch.country === 'pakistan'
                  ? 'PKR '
                  : '£ '}
                {item.total}
          ...
          <TouchableOpacity
            onPress={async () => {
              ...
              fetch(`${myurl}/api/review-item`, requestOptions)
                .then((response) => response.text())
                .then((result) => {
                  let dataa = JSON.parse(result);
                  if (dataa.successData) {
            ...
            style={Styles.OrderAgainView}>
            <Text style={[Styles.SmallText, {color: Pink}]}>{'Rate'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
