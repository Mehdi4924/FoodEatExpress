...
class Topup extends Component {
  ...
  async addwallet() {
    if (this.state.cardnumber == 0 || this.state.expirymonth == 0 || this.state.expiryyear == 0 || this.state.cvc == 0) {
      ...
      //"https://pizzakebab.ranglerztech.website/api/add-wallet"
      fetch(`${myurl}/api/add-wallet`, requestOptions)
   ...
   async getprofile(){
    ...
    fetch(
      ...
      .then(result => {
        let dataa = JSON.parse(result);
        console.log('user data in profile', dataa);
        if (dataa.successData.address&&dataa.successData.address.indexOf('Pakistan') > -1) {
          this.setState({country:'pakistan'}) 
        } else {
            ...
            <View style={styles.card}>
              <View style={{ width: '90%' }}>
                ...
                <View
                  ...
                  >
                  {this.state.country&&this.state.country=='pakistan'?'PKR':'£'}
                  </Text>
                  {/*<Icon
                    name='pound-sign'
                    type='font-awesome-5'
